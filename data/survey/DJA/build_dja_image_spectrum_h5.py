#!/usr/bin/env python3
"""
Build dja_spectra_low_image.h5

For each entry in dja_spectra_low.h5:
  1. Find the grizli F150W mosaic via dja_low_res.csv (srcid → file_phot)
  ...
  6. Write image + ALL spectrum columns from dja_spectra_low.h5 to new h5
...
Usage
-----
    python build_dja_image_h5.py                          # defaults
    python build_dja_image_h5.py --num-workers 16 --out dja_spectra_low_image.h5
"""
...
# The grizli DJA mosaics are already in MJy/sr (JWST pipeline standard),
# identical to COSMOS-Web. No unit conversion is needed.

# All columns in dja_spectra_low.h5 (copied verbatim, filtered to valid rows)
...
def main(argv=None) -> None:
    p = argparse.ArgumentParser(
        description="Build dja_spectra_low_image.h5 with 128x128 F150W cutouts",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--csv",          default="dja_low_res.csv",
                   help="DJA catalog CSV (must contain srcid + file_phot columns)")
    p.add_argument("--h5-in",        default="dja_spectra_low.h5",
                   help="Input spectrum HDF5 (dja_spectra_low.h5)")
    p.add_argument("--mosaic-dir",   default="download_mosaic",
                   help="Directory containing grizli F150W mosaic FITS files")
    p.add_argument("--out",          default="dja_spectra_low_image.h5",
