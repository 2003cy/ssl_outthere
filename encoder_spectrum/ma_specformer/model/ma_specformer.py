...
class MASpecFormer(L.LightningModule):
    ...
    def _wavelength_positional_encoding(
        ...
        """
        # Normalize wavelengths to [0, 1] for stable Fourier encoding
        ...
        # d = embed_dim // 2 frequencies
        num_freqs = embed_dim // 2
        freqs = torch.logspace(0, 8, num_freqs, device=device)  # [1, ..., 1e8]
    ...
    def forward(
        ...
        Args:
            flux: shape (B, T), flux values (may be masked with 0, will be normalized internally)
            wavelengths: shape (B, T), wavelength values
            valid_mask: shape (B, T), bool (True = valid, False = invalid/padding)
                        Invalid positions are masked in attention (cannot be attended to)
    ...
    def _mask_input(self, flux: Tensor, valid_mask: Tensor) -> Tuple[Tensor, Tensor]:
        """Apply random mask to flux values (specformer style).
        
        Only masks positions that are valid (valid_mask=True).
        ...
        Returns:
            masked_flux: shape (B, T), with some valid positions zeroed
            train_mask: shape (B, T), bool (True = position was masked)
        """
        B, T = flux.shape
        ...
        for i in range(B):
            # Only mask among valid positions
