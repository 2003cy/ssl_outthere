"""Transformer modules for ma_specformer.

Includes: LayerNorm, TransformerBlock, SelfAttention, MLP.
Reuses patterns from specformer but simplified for ma_specformer.
"""
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        # Apply key_padding_mask (mask at key positions, last dimension)
        if key_padding_mask is not None:
            # key_padding_mask: (B, T), True where to mask (these are KEY positions)
            # Scores shape: (B, num_heads, T_query, T_key), mask applies to last dim
        ...
        attn = attn.masked_fill(torch.isnan(attn), 0.0)  # Handle NaN from -inf
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        Args:
            x: shape (B, T, D)
            key_padding_mask: shape (B, T), True where to mask
