#!/usr/bin/env python3
"""
DJA spectrum exporter.

Reads a CSV (or masked CSV) built from dja_cat.ipynb, loads .spec.fits files
...
-----
    # single-threaded
    python export_dja_spectrum.py --csv cat_selected.csv --out dja_spectra.h5

    # multi-threaded (4 workers)
    python export_dja_spectrum.py --csv cat_selected.csv --out dja_spectra.h5 --num-workers 4

    # quick test on first 50 rows
    python export_dja_spectrum.py --csv cat_selected.csv --out test.h5 --limit 50
"""
...
# The output lives on a network filesystem (Lustre/NFS) that does not support
# HDF5's file locking; disable it before h5py initializes the HDF5 library,
# otherwise File("w") fails with BlockingIOError (errno 11).
...
def run_multithreaded(
    ...
    """
    # Split by row position; np.array_split on a DataFrame can coerce it to a
    # bare ndarray (losing .iterrows), so split the index instead and use iloc.
    slices = [
        df.iloc[idx] for idx in np.array_split(np.arange(len(df)), num_workers)
    ]
...
def main(argv=None) -> None:
    p = argparse.ArgumentParser(
        description="Export DJA spectra to a single HDF5 file",
