"""Extract CLS embeddings from image and spectrum encoders for DJA fusion training.

Data source: images/DJA/dja_spectra_low_image.h5
  - N aligned (image, spectrum) pairs  (N ≈ 10k-14k after QC)
  - image: (N, 128, 128)  float32  MJy/sr  F150W @ 30mas
  - flux:  (N, 56)        float32  uJy
  - wave:  (N, 56)        float32  micron

Output (written back into dja_spectra_low_image.h5 as new datasets):
...
Usage (from encoder_image/astrodino so sys.path picks up dinov2 + preprocessing):
    ...
    XFORMERS_DISABLED=1 python ../../encoder_fusion/extract_embeddings_dja.py
"""
...
# ── Config ──────────────────────────────────────────────────────────────────
DJA_H5 = PROJECT_ROOT / "images" / "DJA" / "dja_spectra_low_image.h5"
...
SPEC_CKPT   = (PROJECT_ROOT
               ...
               / "dja_prism_f100lp_20260228_151905/ckpt-epoch=143-val_loss=0.3413.ckpt")
...
class DJAImageDataset(Dataset):
    """Load images from dja_spectra_low_image.h5  (flat key: "image")."""
    ...
    def __getitem__(self, idx):
        with h5py.File(self.h5_path, "r") as f:
            img = f["image"][idx].astype("float32")   # (H, W)  MJy/sr
...
class DJASpectrumDataset(Dataset):
    """Load flux/wave from dja_spectra_low_image.h5  (flat keys: "flux", "wave")."""

    def __init__(self, h5_path: Path):
        self.h5_path = h5_path
        with h5py.File(h5_path, "r") as f:
            self.n = f["flux"].shape[0]
    ...
    def __getitem__(self, idx):
        with h5py.File(self.h5_path, "r") as f:
            flux = f["flux"][idx].astype("float32")
            wave = f["wave"][idx].astype("float32")
...
def main():
    ...
    print(f"H5 file: {DJA_H5}")

    with h5py.File(DJA_H5, "r") as f:
        n_total = f["image"].shape[0]
    ...
    img_ds     = DJAImageDataset(DJA_H5, CROP_SIZE, to_rgb)
    ...
    spec_ds     = DJASpectrumDataset(DJA_H5)
    ...
    print(f"\nWriting embeddings → {DJA_H5}")
    with h5py.File(DJA_H5, "a") as f:
        for key, data in [
