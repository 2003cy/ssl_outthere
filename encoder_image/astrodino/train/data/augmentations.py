...
background sigma (``COSMOS_SKY_SIGMA``), with a half-normal scale ``s=|N(0,w)|``
...
``asinh_stretch`` (class ``AsinhStretch``) is a sign-preserving arcsinh stretch
(no clipping at 0).
"""
...
# Measured background sky sigma per COSMOS-Web F150W tile (MJy/sr), from
# images/cosmos_2025/measure_sky_sigma_per_tile.py. Tiles are remarkably uniform
# (2.23e-2 .. 2.40e-2); per-tile kept for future per-field CEERS/OutThere reuse.
COSMOS_SKY_SIGMA = {
    "A1": 2.3078e-02, "A2": 2.3394e-02, "A3": 2.3886e-02, "A4": 2.3568e-02,
    "A5": 2.3977e-02, "A6": 2.3513e-02, "A7": 2.3752e-02, "A8": 2.3749e-02,
    "A9": 2.3616e-02, "A10": 2.3782e-02,
    "B1": 2.3675e-02, "B2": 2.3973e-02, "B3": 2.2299e-02, "B4": 2.3839e-02,
    "B5": 2.3358e-02, "B6": 2.3666e-02, "B7": 2.3617e-02, "B8": 2.3426e-02,
    "B9": 2.3338e-02, "B10": 2.3654e-02,
}
SKY_SIGMA_FALLBACK = float(np.median(list(COSMOS_SKY_SIGMA.values())))
...
class DataAugmentationAstroDINO(object):
    def __init__(
        ...
        print("###################################")
        print(f"model input size: {global_crops_size}x{global_crops_size}")
        print("####################################")
    ...
    def __call__(self, image, tile: str):
        sigma_sky = COSMOS_SKY_SIGMA.get(tile, SKY_SIGMA_FALLBACK)
        output = {}
...
class GaussianNoise:
    ...
    def __init__(self, mean: float = 0.0, w: float = 2, s_max: float | None = None):
        self.mean = mean
        self.w = w
        self.s_max = s_max
