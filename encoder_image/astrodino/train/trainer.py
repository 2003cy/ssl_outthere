# AstroDino trainer script, taken and modified from DINOv2 trainer and astroclip repo: 
...
def do_test(cfg, model, iteration, run_name):
...
def do_train(cfg, model, run_name, group_name, resume=False):
    ...
    data_transform = DataAugmentationAstroDINO(
        ...
        center_crop_size=cfg.crops.center_crop_size,
    )
    ...
    for data in metric_logger.log_every(data_loader, 25, header, max_iter, start_iter):
        ...
        if (
            ...
            do_test(cfg, model, f"training_{iteration}", run_name)
...
def main_cli(cli_args=None):
    ...
    if args.eval_only:
        ...
        return do_test(cfg, model, f"manual_{iteration}", run_name)
